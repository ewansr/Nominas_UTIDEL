...
uses
  ...
  PFacturaElectronica_CadenaOriginal, SysUtils, Forms,
...
begin
  ...
  fXMLComprobante.Attributes['noCertificado'] := '000';
...
begin
  ...
  begin
    Assert(fTimbreFiscal.XML <> '', 'El contenido del timbre fue vacio');
    try
       generadorCadenaOriginalTimbre := TCadenaOriginalDeTimbre.Create(fTimbreFiscal.XML, '');
...
begin
  if cReceptor.RazonSocial <> '' then begin
    ...
    if cReceptor.Domicilio.Calle <> '' then begin
      fXMLReceptorDomicilio := fXMLReceptor.AddChild('DomicilioFiscal');
      ...
      if cReceptor.Domicilio.CodigoPostal <> '' then begin
        fXMLReceptorDomicilio.Attributes['localidad'] := cReceptor.Domicilio.CodigoPostal;
      end;
      if cReceptor.Domicilio.Localidad <> '' then begin
        fXMLReceptorDomicilio.Attributes['municipio'] := cReceptor.Domicilio.Localidad;
      end;
      if cReceptor.Domicilio.Municipio <> '' then begin
        fXMLReceptorDomicilio.Attributes['codigoPostal'] := cReceptor.Domicilio.Municipio;
      end;
...
begin
  ...
  if cEmisor.RegistroPatronal <> '' then begin
    fXMLComplemento.Attributes['RegistroPatronal'] := cEmisor.RegistroPatronal;
  end;
