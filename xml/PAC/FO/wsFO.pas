...
//  layoutB64 := UnicodeStr;

  //Vamos a verificar la calabaza que me envian estos weyes
  ...
  xmlString := '';
  xmlString := layout;
  AssignFile(archivoTxt, 'XMLEnvelopeXML.txt');
  try
    Rewrite(archivoTXT);
    Writeln(archivoTXT, xmlString);
  finally
  ...
  XMLDOM := CreateOleObject('MSXML2.DOMDocument.3.0') as IXMLDOMDocument2;
