...
var
  SL : TStringList;
begin
  // Llenar el combo de las webcam
  SL := TStringList.Create;
  VideoImage.GetListOfDevices(SL);
  ComboBox_Cams.Items.Assign(SL);
  SL.Free;

  //al encontrar video habilitar el boton
...
//     img2.Canvas.Draw(0, 0, VideoBMP[VideoBMPIndex]);
     w := (img2.Width/2)-(VideoImage.VideoWidth/2);
     h := (img2.Height/2)-(VideoImage.VideoHeight/2);
     img2.Canvas.Draw(Trunc(w), Trunc(h), VideoBMP[VideoBMPIndex]);
   //  (img2.Width/2)-(VideoImage.VideoWidth/2);
     begin
      DiffRatio := 0;
      //PaintBox_Video.Canvas.Draw(0, 0, ModeBMP);
      img2.Canvas.Draw(0, 0, ModeBMP);
      // Surveillance
...
begin
  ...
  // actualizar la lista de camaras
  UpdateCamList;
...
begin
  ...
  UpdateCamList;
{  w := PaintBox_Video.Width;
  h :=PaintBox_Video.Height;

  img2.Canvas.CopyRect(Rect(0,0,w,h),PaintBox_Video.Canvas, Rect(0,0,w,h));  }
end;
