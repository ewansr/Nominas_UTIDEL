...
type
  TFrmReporteOCSinEnt = class(TForm)
...
var
  FrmReporteOCSinEnt: TFrmReporteOCSinEnt;
...
procedure TFrmReporteOCSinEnt.AdvGlowButton1Click(Sender: TObject);
Var
...
begin
  try
    ...
    CrearConjunto(cdDatos,'rep_ocmaterialsinentrada',ccSelect);
    ...
    CrearConjunto(cdDetalle,'rep_ocmaterialsinentradadetalle',ccSelect);
    ...
    //Asignar archivo
    frxReporte.LoadFromFile(ExtractFilePath(Application.ExeName)+'\Reportes\'+'OCMaterialesSinEntrada.fr3');
...
procedure TFrmReporteOCSinEnt.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Globales.SetValue('OrgPredeterminada',CmbConvenio.KeyValue);
end;

procedure TFrmReporteOCSinEnt.FormShow(Sender: TObject);
Var
  Hoy: TDate;
begin
...
procedure TFrmReporteOCSinEnt.EstableceIndex(Data:TclientDataset;Combo:TDBLookupComboBox);
begin
  try
...
procedure TFrmReporteOCSinEnt.CargarConvenios(Refrescar: Boolean);
var
  i: integer;
begin
