unit UTfrmReporteMaterialNoCotizado;
...
type
  TFrmReporteMatNoCot = class(TForm)
...
var
  FrmReporteMatNoCot: TFrmReporteMatNoCot;
...
procedure TFrmReporteMatNoCot.AdvGlowButton1Click(Sender: TObject);
Var
...
begin
  try
    ...
    CrearConjunto(cdDatos,'rep_solicitudmaterialnocotizado',ccSelect);
    ...
    CrearConjunto(cdDetalle,'rep_solicitudmaterialnocotizadodetalle',ccSelect);
    ...
    //Asignar archivo
    frxReporte.LoadFromFile(ExtractFilePath(Application.ExeName)+'\Reportes\'+'SolicitudMaterialesNoCotizados.fr3');
...
procedure TFrmReporteMatNoCot.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Globales.SetValue('OrgPredeterminada',CmbConvenio.KeyValue);
end;

procedure TFrmReporteMatNoCot.FormShow(Sender: TObject);
Var
  Hoy: TDate;
begin
...
procedure TFrmReporteMatNoCot.FiltroCombo;
begin

end;

procedure TFrmReporteMatNoCot.EstableceIndex(Data:TclientDataset;Combo:TDBLookupComboBox);
begin
  try
...
procedure TFrmReporteMatNoCot.CargarConvenios(Refrescar: Boolean);
var
  i: integer;
begin
