...
type
  TFrmSolicitudPartida = class(TFrmPartidaDoc)
    btnArticulo: TButton;
...
var
  FrmSolicitudPartida: TFrmSolicitudPartida;
...
procedure TFrmSolicitudPartida.CargarCampos;
begin

end;

function TFrmSolicitudPartida.ValidaCampos: Boolean;
begin
  result := true;
end;

procedure TFrmSolicitudPartida.LimpiarCampos;
begin
  edtCodigoArticulo.Text := '';
  InicializarCampos;
end;

procedure TFrmSolicitudPartida.InicializarCampos;
begin
  mdDetalle.FieldByName('cantidad').AsFloat := 1.0;
  mdDetalle.FieldByName('fecharequerida').AsDateTime := fechaRequeridaDefault;
end;

procedure TFrmSolicitudPartida.BtnAddClick(Sender: TObject);
begin
  inherited;
  edtCodigoArticulo.SetFocus;
end;

procedure TFrmSolicitudPartida.btnArticuloClick(Sender: TObject);
var
  Articulo: TLista;
...
begin
  ...
  frmArticulo.Filtro := edtCodigoArticulo.Text;
  ...
  begin
    ...
    dsDetalle.DataSet.FieldValues['descripcionarticulo'] := Articulo.Valor['descripcion'];
...
procedure TFrmSolicitudPartida.edtCantidadKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if key = #13 then
...
procedure TFrmSolicitudPartida.edtCodigoArticuloKeyPress(Sender: TObject;
  var Key: Char);
var
  ProviderName: String;
...
begin
  ...
  begin
    ...
    begin
      ...
      mdDetalle.FieldByName('descripcionarticulo').AsString := cdArticulo.FieldByName('descripcion').AsString;
      Encontrado := true;
    end;
    cdArticulo.Close;
    cdArticulo.Free;
...
procedure TFrmSolicitudPartida.FormCreate(Sender: TObject);
begin
  inherited;
