unit UTfrmReporteMaterialSinEntrada;
...
type
  TFrmReporteMatSinEnt = class(TForm)
...
var
  FrmReporteMatSinEnt: TFrmReporteMatSinEnt;
...
procedure TFrmReporteMatSinEnt.AdvGlowButton1Click(Sender: TObject);
Var
...
begin
  try
    ...
    CrearConjunto(cdDatos,'rep_solicitudmaterialsinentrada',ccSelect);
    ...
    CrearConjunto(cdDetalle,'rep_solicitudmaterialsinentradadetalle',ccSelect);
    ...
    //Asignar archivo
    frxReporte.LoadFromFile(ExtractFilePath(Application.ExeName)+'\Reportes\'+'SolicitudMaterialesSinEntrada.fr3');
...
procedure TFrmReporteMatSinEnt.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Globales.SetValue('OrgPredeterminada',CmbConvenio.KeyValue);
end;

procedure TFrmReporteMatSinEnt.FormShow(Sender: TObject);
Var
  Hoy: TDate;
begin
...
procedure TFrmReporteMatSinEnt.FiltroCombo;
begin

end;

procedure TFrmReporteMatSinEnt.EstableceIndex(Data:TclientDataset;Combo:TDBLookupComboBox);
begin
  try
...
procedure TFrmReporteMatSinEnt.CargarConvenios(Refrescar: Boolean);
var
  i: integer;
begin
