...
type
  TFrmRequisicionPartida = class(TFrmPartidaDoc)
    btnArticulo: TButton;
...
var
  FrmRequisicionPartida: TFrmRequisicionPartida;
...
procedure TFrmRequisicionPartida.CargarCampos;
begin

end;

function TFrmRequisicionPartida.ValidaCampos: Boolean;
begin
  result := true;
end;

procedure TFrmRequisicionPartida.LimpiarCampos;
begin
  edtCodigoArticulo.Text := '';
  InicializarCampos;
end;

procedure TFrmRequisicionPartida.InicializarCampos;
begin
  mdDetalle.FieldByName('cantidad').AsFloat := 1.0;
  mdDetalle.FieldByName('fecharequerida').AsDateTime := fechaRequeridaDefault;
end;

procedure TFrmRequisicionPartida.BtnAddClick(Sender: TObject);
begin
  inherited;
  edtCodigoArticulo.SetFocus;
end;

procedure TFrmRequisicionPartida.btnArticuloClick(Sender: TObject);
var
  Articulo: TLista;
...
procedure TFrmRequisicionPartida.edtCantidadKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if key = #13 then
...
procedure TFrmRequisicionPartida.edtCodigoArticuloKeyPress(Sender: TObject;
  var Key: Char);
var
  ProviderName: String;
...
procedure TFrmRequisicionPartida.FormCreate(Sender: TObject);
begin
  inherited;
