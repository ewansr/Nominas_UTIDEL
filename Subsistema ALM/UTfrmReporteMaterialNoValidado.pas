unit UTfrmReporteMaterialNoValidado;
...
type
  TFrmReporteMatNoVal = class(TForm)
...
var
  FrmReporteMatNoVal: TFrmReporteMatNoVal;
...
procedure TFrmReporteMatNoVal.AdvGlowButton1Click(Sender: TObject);
Var
...
begin
  try
    ...
    CrearConjunto(cdDatos,'rep_solicitudmaterialnovalidado',ccSelect);
    ...
    CrearConjunto(cdDetalle,'rep_solicitudmaterialnovalidadodetalle',ccSelect);
    ...
    //Asignar archivo
    frxReporte.LoadFromFile(ExtractFilePath(Application.ExeName)+'\Reportes\'+'SolicitudMaterialesNoValidados.fr3');
...
procedure TFrmReporteMatNoVal.FiltroCombo;
begin

end;

procedure TFrmReporteMatNoVal.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Globales.SetValue('OrgPredeterminada',CmbConvenio.KeyValue);
end;

procedure TFrmReporteMatNoVal.FormShow(Sender: TObject);
Var
  Hoy: TDate;
begin
...
procedure TFrmReporteMatNoVal.EstableceIndex(Data:TclientDataset;Combo:TDBLookupComboBox);
begin
  try
...
procedure TFrmReporteMatNoVal.CargarConvenios(Refrescar: Boolean);
var
  i: integer;
begin
