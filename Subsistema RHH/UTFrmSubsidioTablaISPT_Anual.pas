  ...
  begin
    ...
    Try
      ...
      if Not CargarDatosFiltrados(cdTablaISR, 'FechaAplicacion', [sFecha]) then
        raise InteligentException.CreateByCode(6, ['Tipo de Tabla de ISPT/Fecha de Aplicación', cdTablaISR.FieldByName('IdTipoTabla').AsString + '/' + sFecha]);
      cdTablaISR.Open;
    Finally
  ...
  begin
    ...
    Try
      ...
      if Not CargarDatosFiltrados(cdTablaISR, 'FechaAplicacion', [-1]) then
        raise InteligentException.CreateByCode(6, ['Tabla Anual de ISPT/Fecha de Aplicación', cdTablaISR.FieldByName('FechaAplicacion').AsString + '/' + sFecha]);
      cdTablaISR.Open;
    Finally
      Screen.Cursor := Cursor;
    End;
