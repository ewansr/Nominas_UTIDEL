...
type
  TFrmTablaISPTAnual = class(TForm)
    Panel1: TPanel;
...
var
  FrmTablaISPTAnual: TFrmTablaISPTAnual;

implementation

{$R *.dfm}

procedure TFrmTablaISPTAnual.cbTipoTablaClick(Sender: TObject);
var
  sFecha: String;
  Anio, Mes, Dia: Word;
  Cursor: TCursor;
...
procedure TFrmTablaISPTAnual.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  try
...
procedure TFrmTablaISPTAnual.FormCreate(Sender: TObject);
var
  Cursor: TCursor;
begin
  Actualizar := False;
  ...
  Try
    ...
    Try
      ...
      if Not CrearConjunto(cdHistorial, 'nom_tablaisptanual', ccSelect) then
        raise InteligentException.CreateByCode(5, ['Historial de la Tabla del Impuesto Sobre el Producto del Trabajo (ISPT) anual']);
      cdHistorial.Open;

      if Not CrearConjunto(cdTablaISPTAnual, 'nom_tablaisptanual', ccCatalog) then
        raise InteligentException.CreateByCode(5, ['Tabla del Impuesto Sobre el Producto del Trabajo (ISPT) anual']);
    Finally
      Screen.Cursor := Cursor;
    End;
  Except
...
procedure TFrmTablaISPTAnual.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
     27: Close;
...
procedure TFrmTablaISPTAnual.FormShow(Sender: TObject);
begin
...
procedure TFrmTablaISPTAnual.tsPeriodosChange(Sender: TObject; NewTab: Integer;
  var AllowChange: Boolean);
var
  Cursor: TCursor;
begin
...
procedure TFrmTablaISPTAnual.LeerDatos;
Const
...
procedure TFrmTablaISPTAnual.NextDBGrid1CellDblClick(Sender: TObject; ACol,
  ARow: Integer);
begin
  Btn_EditarClick(Btn_Editar);
end;

procedure TFrmTablaISPTAnual.Btn_AgregarClick(Sender: TObject);
var
  Resultado: Integer;
begin
  Try
    ...
    begin
      ...
      begin
        ...
        if Not CrearConjunto(Buscar, 'nom_tablaisptanual', ccUpdate) then
          raise InteligentException.CreateByCode(5, ['Tabla de I.S.P.T. anual']);
      end;
    ...
    FrmCaptura.Caption := 'Captura datos de nuevo renglón de tabla de I.S.P.T. anual';
    Try
...
procedure TFrmTablaISPTAnual.Btn_AgregarLineaClick(Sender: TObject);
...
begin
  Try
    ...
    Try
      ...
      begin
        Buscar.close;
        If Not CargarDatosFiltrados(Buscar, 'IdTablaISPT', [cdTablaISPTAnual.FieldByName('IdTablaISPT').AsInteger]) then
          raise Exception.Create('No se ha podido acceder a la tabla de I.S.P.T.');
        Buscar.Open;
        Buscar.Edit;
      end;
...
procedure TFrmTablaISPTAnual.Btn_CerrarLineaClick(Sender: TObject);
begin
  if Assigned(FrmCaptura) then
    FrmCaptura.Close;
end;

procedure TFrmTablaISPTAnual.Btn_EditarClick(Sender: TObject);
Var
  i,Resultado: integer;
begin
  try
    ...
    eCuotaFija.Text := cdTablaISPTAnual.FieldByName('cuotafija').AsString;
    epseli.Text := cdTablaISPTAnual.FieldByName('pseli').AsString;
    FrmCaptura.Caption := 'Edicion de  datos de tabla de I.S.P.T. anual';
...
procedure TFrmTablaISPTAnual.Btn_QuitarClick(Sender: TObject);
var
  Cursor: TCursor;
begin
  ...
  Try
    ...
    Try
      ...
      Buscar.Close;
      If Not CargarDatosFiltrados(Buscar, 'IdTablaISPT', [cdTablaISPTAnual.FieldByName('IdTablaISPT').AsInteger]) then
        raise InteligentException.CreateByCode(6, ['Tabla I.S.P.T.', 'IdTablaISPT',  cdTablaISPTAnual.FieldByName('IdTablaISPT').AsString]);
      Buscar.Open;
    Finally
      Screen.Cursor := Cursor;
    End;

    if (Buscar.RecordCount = 1) and (InteliDialog.ShowModal('Eliminar Línea de Tabla de I.S.P.T. anual', '¿Está seguro que desea eliminar está línea de la tabla del Impuesto Sobre el Producto del Trabajo anual?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
    begin
      Buscar.Delete;
      Buscar.ApplyUpdates(-1);
...
procedure TFrmTablaISPTAnual.Btn_RefreshClick(Sender: TObject);
begin
  try
    cdHistorial.Close;
...
procedure TFrmTablaISPTAnual.ValidaCaptura;
begin
  Try
...
procedure TFrmTablaISPTAnual.CrearVentana;
begin
...
procedure TFrmTablaISPTAnual.DxBLbuttonSalirClick(Sender: TObject);
begin
  Close;
end;
