...
type
  ...
  private
    ...
    procedure pEnabled(Value: Boolean);
  public
    ...
    property Enabled: Boolean read FEnabled write pEnabled;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    procedure AddMenuItem(ACaption: string; AHint: string = ''; AId: Integer = -1; AIconIndex: Integer = -1); overload;
    ...
    procedure AddMenuItem(ACaption: string; AProcedure: TProc; AHint: string = ''; AId: Integer = -1; AIconIndex: Integer = -1); overload;  // MRV
    ...
    procedure AddCheckBox(ACaption: string; AProcedure: TProc; AHint: string = ''; AId: Integer = -1; AChecked: boolean = false; AEnabled: Boolean = true; AFlat: boolean = false); overload;
    procedure AddBmp(AImage: TBitmap); overload;
    procedure AddBmp(AImageFile: TFileName); overload;
    procedure AddImage(AField: TField); overload;
    procedure AddImage(AStream: TStream); overload;
  ...
  public
    ...
    property PointClick: TPoint read FPointClick;
  published
    ...
    property ProcList: TStringListX read FProcList write FProcList;
...
begin
    ...
    begin
      ...
      begin
        Font.Color := IfThenColor(Self = FOwner.FExBar.FHotArea, FOwner.FExBar.FTextHotColor, clGray);
        Font.Style := IfThenFs(Self = FOwner.FExBar.FHotArea, [], []);
      end;
      IconTop := Y;
      if FIcon <> nil then
      begin
      ...
      repeat
        ...
        if {(FEnabled) and} (X + IconWidth + TextWidth(Aline) > FClipRect.Right) then
...
procedure TExMenuItem.pEnabled(Value: Boolean);
begin
  FEnabled := Value;
  Self.Owner.FExBar.Parent.Repaint;
end;
...
procedure TExGroup.AddMenuItem(ACaption, AHint: string; AId: Integer; AIconIndex: Integer);
begin
  if Assigned(FExBar.FItemIcons) then
    FItems.Add(TExMenuItem.Create(Self, ACaption, AHint, AId, AIconIndex));
end;
...
procedure TExGroup.AddMenuItem(ACaption: string; AProcedure: TProc; AHint: string; AId: Integer; AIconIndex: Integer);
begin
  if Assigned(FExBar.FItemIcons) then
  Begin
    FItems.Add(TExMenuItem.Create(Self, ACaption, AHint, AId, AIconIndex));
    FExBar.ProcList.Add(AId, AProcedure);
  End;
end;
...
procedure TExGroup.AddImage(AField: TField);
begin
  AddImage(AField.DataSet.CreateBlobStream(AField, bmRead));
end;

{-----------------------------------------------}

procedure TExGroup.AddImage(AStream: TStream);
var
  Jp: TJPegImage;
  ic: TIcon;
  LocBitMap: TBitMap;
  ...
  begin
    ...
    case bf.LoWord of
      $4D42: begin     // BMP
        ...
        LocBitMap.LoadFromStream(AStream);
      end;
      $0000: begin   // icono
        ic := TIcon.Create;
    ...
    FItems.Add(TExBmp.Create(Self, LocBitmap));
    //Self.AddBmp(LocBitmap);
  end;
end;
...
procedure TExGroup.AddCheckBox(ACaption: string; AProcedure: TProc; AHint: string = ''; AId: Integer = -1; AChecked: boolean = false; AEnabled: Boolean = true; AFlat: boolean = false);
begin
  FItems.Add(TExCheckBox.Create(Self, ACaption, AId, AHint, AChecked, AEnabled, AFlat));
  FExBar.ProcList.Add(AId, AProcedure);
end;
